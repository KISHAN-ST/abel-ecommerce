...
/** Response shape when loading from static JSON (mock backend) */
interface ProductsJsonResponse {
  products: (Omit<Product, 'releaseDate'> & { releaseDate: string })[];
  categories: Category[];
}
...
export class ProductService {
  ...
  /** Load products and categories from static JSON file (mock backend); fallback to inline mock on error */
  private loadProductsFromJson(): void {
    this.http
      .get<ProductsJsonResponse>('/data/products.json')
      .pipe(
        tap((data) => {
  ...
  /** Add a new product to the catalog (admin simulation). Assigns new id and pushes to stream. */
  addProduct(input: Omit<Product, 'id'>): Product {
    const id = String(Math.max(0, ...this.products.map(p => parseInt(p.id, 10))) + 1);
    const product: Product = {
      ...input,
      id,
      releaseDate: input.releaseDate instanceof Date ? input.releaseDate : new Date(input.releaseDate)
    };
